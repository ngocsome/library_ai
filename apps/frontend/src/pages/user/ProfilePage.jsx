...
const getEmail = (profile) =>
  profile?.Email || profile?.email || 'Chưa cập nhật email';
...
const EditProfileModal = ({
  ...
  return createPortal(
    <AnimatePresence>
      <div className="fixed inset-0 z-[2147483647] flex items-center justify-center bg-slate-950/60 backdrop-blur-lg p-4">
        <motion.div
          initial={{ opacity: 0, y: 40, scale: 0.95 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          exit={{ opacity: 0, y: 40, scale: 0.95 }}
          transition={{ type: 'spring', stiffness: 350, damping: 28 }}
          className="w-full max-w-xl bg-white rounded-[32px] shadow-[0_25px_60px_rgba(0,0,0,0.35)] border border-white/20 overflow-hidden"
        >
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Cập nhật thông tin cá nhân của bạn trên hệ thống.
                </p>
              </div>
            </div>
          ...
          <div className="p-6 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Vui lòng thiết lập mật khẩu mạnh để bảo vệ tài khoản.
                </p>
              </div>
            </div>
...
const ProfilePage = () => {
  ...
  const openEditModal = () => {
    setEditForm({
      fullName:
        getFullName(profile) === 'Chưa cập nhật tên'
          ? ''
          : getFullName(profile),
      email:
        getEmail(profile) === 'Chưa cập nhật email' ? '' : getEmail(profile),
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-12 relative z-10 px-4 sm:px-6">
      <div className="bg-white rounded-[32px] shadow-[0_8px_40px_rgba(15,23,42,0.03)] border border-slate-200/60 overflow-hidden relative">
        <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-500 overflow-hidden">
          <div
        ...
        <div className="relative pt-16 pb-8 px-8 flex flex-col lg:flex-row items-center lg:items-end justify-between gap-6">
          <div className="flex flex-col md:flex-row items-center md:items-end gap-6 text-center md:text-left w-full lg:w-auto">
            <div className="relative shrink-0 -mt-10 md:-mt-12 z-10">
          ...
          <div className="flex flex-row gap-3 w-full sm:w-auto shrink-0 justify-center pb-2">
            <button
              type="button"
              onClick={openPasswordModal}
              className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-5 py-3 bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 rounded-2xl font-bold text-xs transition-all cursor-pointer active:scale-[0.97] shadow-sm hover:shadow-md"
            >
        ...
        <div className="space-y-2 shrink-0 bg-white/60 backdrop-blur-md p-2 rounded-3xl border border-slate-200/40 shadow-sm">
          ...
          >
            <User
              size={15}
              className={
                activeTab === 'info' ? 'text-brand-green' : 'text-slate-400'
              }
            />
            Thông tin chung
          </button>
          ...
          {activeTab === 'info' && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-2xl bg-slate-50/50 hover:bg-slate-50 hover:shadow-sm border border-slate-200/40 hover:border-slate-200 transition-all duration-300 flex items-start gap-4 md:col-span-2">
            ...
            >
              ...
              <div className="relative pl-6 space-y-6 before:absolute before:left-[11px] before:top-2.5 before:bottom-2.5 before:w-[1.5px] before:bg-slate-200/80">
                {activities.length > 0 ? (
                  activities.map((act, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          <p className="text-slate-800 font-bold text-sm tracking-tight leading-none mb-1.5">
                            {title}
                          </p>

                          {description && (
                            <p className="text-slate-500 text-xs font-semibold mb-2 leading-relaxed">
                              {description}
                            </p>
                          )}
                  ...
                  <p className="text-slate-500 text-sm font-semibold pl-2">
                    Chưa có hoạt động nào được ghi nhận trên hệ thống.
                  </p>
                )}
              </div>
            </motion.div>
