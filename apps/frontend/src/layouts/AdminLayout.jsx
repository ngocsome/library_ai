...
const AdminLayout = () => {
  ...
  const getInitialName = () => {
    const name = user?.FullName || user?.fullName || user?.name || 'Quản trị viên';
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-slate-800/40 bg-slate-950/40 backdrop-blur-md">
            <div
              ...
              } p-2 rounded-2xl bg-slate-900/40 border border-slate-800/50 hover:bg-slate-900/70 transition-all duration-300`}
            >
              ...
              {!isCollapsed && (
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-white truncate leading-none mb-1">
                    {user?.FullName || user?.fullName || user?.name || 'Quản trị viên'}
                  </p>
                  <p className="text-[10px] font-medium text-slate-500 tracking-wider uppercase leading-none">
                    Admin
                  </p>
                </div>
              )}
            ...
            <button
              ...
              } py-3 text-slate-400 hover:text-red-400 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 rounded-xl transition-all duration-300 group`}
            >
              <LogOut size={18} className="group-hover:translate-x-0.5 transition-transform" />
              {!isCollapsed && (
                <span className="text-xs font-semibold tracking-wide">Đăng xuất</span>
              )}
            </button>
          </div>
        </div>
